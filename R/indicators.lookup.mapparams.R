...
indicators.lookup.mapparams = function( DS="datarange", vn ){
  ...
  if (DS=="datarange") {
    datarange = NULL
    if ( vn == "ca1" ) datarange = seq(-1, 1, length.out=100)
    if ( vn == "ca2" ) datarange = seq(-1, 1, length.out=100)
    if ( vn == "pca1" ) datarange = seq(-1, 1, length.out=100)
    if ( vn == "pca2" ) datarange = seq(-1, 1, length.out=100)
